...
#' head(dt_ABCA)
#'
#' dt_EOS = EN_Ticker_hcData("EOS", from = "2025-08-25")
#' head(dt_EOS)
#'
...
#' \donttest{
#'
#' # Get hc data for ETF AAPL from August 15, 2025 to now
#' hc_aapl <- EN_Ticker_hcData("AAPL", stock_type = "E", from = "2025-08-15", to = Sys.Date())
#' tail(hc_aapl)
#'
  ...
  Ready.HcData.func <- function(ticker, escape = FALSE, stock_type = 'Eq_Ind', from = NULL, to = Sys.Date()){
    ...
    if (escape %in% c(TRUE, FALSE)) {
      if(escape == TRUE){
        ...
        #               the_adn,
        #               "/max")
        url_ <- paste0("https://live.euronext.com/en/product/equities/",
                       the_adn)
        ...
        run_time_ = b$Runtime$evaluate("function fetch_chart_data(url) {return fetch(url).then(response => response.json()).then(json => ajax_secure_dataFilter(json, 'json', false))}")
        ...
        if (run_time_$result$type == "undefined") {
          ...
          # Créez une liste avec vos éléments
          chart_data_list <- list(
            Date = sapply(chart_data, `[[`, 1),
            Price = sapply(chart_data, `[[`, 2),
            Volume = sapply(chart_data, `[[`, 3)
          )

          data <- as.data.frame(chart_data_list)
