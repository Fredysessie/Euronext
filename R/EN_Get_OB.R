...
#' # print(MSFT_OB)
#'
#' HBAR_OB = EN_Get_OB("HBAR", stock_type = 'Etf')
#' print(HBAR_OB)
#'
...
EN_Get_OB <- function(ticker, stock_type = 'Equity') {
  ticker <- tolower(ticker)
  stock_type <- tolower(stock_type)

  if (length(ticker) != 1) stop("Please provide a unique ticker.")
  if (length(stock_type) != 1) stop("Please provide a unique stock_type.")
  ...
  if (length(test_data) != 1) {
    ...
    # 🔧 Conversion sécurisée : seules les colonnes numériques
    df <- df %>%
      mutate(
        B_order = suppressWarnings(as.numeric(B_order)),
        B_quantity = suppressWarnings(as.numeric(B_quantity)),
        Bid_Price = suppressWarnings(as.numeric(Bid_Price)),
        Ask_Price = suppressWarnings(as.numeric(Ask_Price)),
        A_quantity = suppressWarnings(as.numeric(A_quantity)),
        A_order = suppressWarnings(as.numeric(A_order))
      )

    len_df <- nrow(df)
    add_row_df <- len_df + 1

    # Calculs en ignorant les NA
    df[add_row_df, ] <- NA
    df[add_row_df, "B_order"]   <- sum(df$B_order, na.rm = TRUE)
    df[add_row_df, "B_quantity"]<- sum(df$B_quantity, na.rm = TRUE)
    df[add_row_df, "Bid_Price"] <- "-"
    df[add_row_df, "Ask_Price"] <- "-"
    df[add_row_df, "A_quantity"]<- sum(df$A_quantity, na.rm = TRUE)
    df[add_row_df, "A_order"]   <- sum(df$A_order, na.rm = TRUE)
